                                       ...
                                       l1=1, l2=1, lR=1)
{
  ...
  Salida <- nnls(A, b)
  penalty <- lambda * ((Salida$deviance)/(1e-3+sum((Salida$x - c(lR/l1, lR/l2))^2)))
  penalty <- sqrt(penalty)
  Ab <- rbind(A, c(penalty, 0), c(0, penalty))
  bb <- c(SignalR, penalty*lR/l1, penalty*lR/l2)
  resultado <- nnls(crossprod(Ab), crossprod(Ab,bb))
  u <- resultado$x[1]
  v <- resultado$x[2]
  T1est <- Signal1 * u
  T2est <- Signal2 * v
  if(F) {
    ...
    # w <- c(1,1,1)
    D <- cbind(c(w[1]/(u+.001),0,w[3]),c(0,w[2]/(v+.001),w[3]))
    S <- matnnls(D, Signals*w)
    T1est <- S[1,]
    T2est <- S[2,]
  }
  return(list(T1est = T1est, T2est = T2est))
...
#   T2est <- Signal2 * v
#   
#   D <- cbind(c(1/(u+.000001),0,1),c(0,1/(v+.000001),1))
#   Abig <- rbind(t(A),b)
#   # w <- sqrt(rowMeans(Abig)+1e-4)
#   w <- 1
#   S <- nnls.solvefast(D*w, Abig*w)
#   T1est <- S[1,]
#   T2est <- S[2,]
#   
#   return(list(T1est = T1est, T2est = T2est))
# }
# 
# nnls.solve <- function(A,B) {
#   coeficientes <- matrix(NA, ncol(A), ncol(B))
#   for (n in 1:ncol(B)) {
#     b <- B[,n]
#     salida <- nnls(A,b)
#     coeficientes[,n] <- salida$x
#   }
#   return(coeficientes)
# }
# 
# 
# 
# nnls.solvefast <- function(A,B) {
#   library(matrixStats)
#   coeficientes <- matrix(NA, ncol(A), ncol(B))
#   coeficientes <- solve(crossprod(A), crossprod(A,B))
#   
#   # Fix negative values for first path
#   # Fix negative values for second path
#   
#   if (min(coeficientes) < 0) {
#     coeficientes[,which(colMins(coeficientes) <0)] <- nnls.solve(A, B[,which(colMins(coeficientes) <0), drop = F])
#   }
#   
#   return(coeficientes)
# }
...
estimatePSI <- function(CountMatrix, l1eq, l2eq, lReq, lambda = NULL) {
  if (is.null(lambda)) {
    lambda <- 0.1
  }
  # print(lambda)
  PSI <- matrix(NA, ncol = ncol(CountMatrix), nrow = nrow(CountMatrix)/3)
  for (n in seq_len(nrow(CountMatrix)/3)) {
    Signal1 <- CountMatrix[1 + 3 * (n - 1), ]
    Signal2 <- CountMatrix[2 + 3 * (n - 1), ]
    SignalR <- CountMatrix[3 + 3 * (n - 1), ]
    
    Output <- estimateAbsoluteConc_boot(Signal1, Signal2, SignalR, 
                                   lambda,
                                   l1eq[n], l2eq[n], lReq[n])
...
calcBootstrapPSI <- function(rowProcess, PSI_boot, CountMatrix, l1eq, l2eq, lReq, lambda, nboot) {
  library(nnls)
  ...
  for (n in rowProcess) {
    ...
    Signal2 <- CountMatrix[2 + 3 * (n - 1), ]
    SignalR <- CountMatrix[3 + 3 * (n - 1), ]
    ST <- Signal1+Signal2+SignalR
    M <- apply(cbind(Signal1, Signal2, SignalR), 1,
               function(x) {
                 ST <- x[1] + x[2] + x[3]
                 rmultinom(nboot, ST, prob = c((x[1]+1e-4)/(ST+1e-4), (x[2]+1e-4)/(ST+1e-4), (x[3]+1e-4)/(ST+1e-4)))
               }
    )
    # M is a matrix of nboot * 3 * number of samples
    
    for (m in 1:nboot) {
      Signal1b <- M[3*(m-1)+1,]
      Signal2b <- M[3*(m-1)+2,]
      SignalRb <- M[3*(m-1)+3,]
      Output <- estimateAbsoluteConc_boot(Signal1b, 
                                     Signal2b, SignalRb, lambda,
                                     l1eq[n], l2eq[n], lReq[n])
      psib <- Output$T1est/(Output$T1est + Output$T2est)
      PSI_boot[,n,m] <- psib
    }
    
    # if(PLOT) {
    #   boxplot(t(PSI_boot[,n,]))
    #   lines(psi, col = "red", pch = 16, type = "b")
    #   lines(rowMeans(PSI_boot[,n,]), col = "dark green", pch = 16, type = "b")
    # }
...
replaceNAN <- function(PSI_boot, Design){
...
mclapplyPSI_Bootstrap <- function(PSI_boots,Design,Contrast,cores,ram,nbootstraps,KallistoBootstrap,th, verbose = 0){
  ...
  groups <- round(t(V) %*% rnorm(nrow(V)),digits = 6)
  uniqueGroups <- unique(groups)
  # tableGroups <- data.table::as.data.table(groups)[,list(list(.I)),by = groups]
  for (event in 1:dim(PSI_boots)[1]) {
    valPSI_Event <- PSI_boots[event,,]
    if (is.na(sum(valPSI_Event))) {
      for (group in 1:length(uniqueGroups)) {
        group <- which(uniqueGroups[group] == groups)
        valPSI_E_Group <- PSI_boots[event,,group]
        if (is.na(sum(valPSI_E_Group))) {
          AllNa <- length(unique(is.na(as.vector(valPSI_E_Group)))) == 1
          if(!AllNa){
            posNA <- which(is.na(valPSI_E_Group))
            valSameGroup <- as.vector(valPSI_E_Group[!is.na(valPSI_E_Group)])
            valPSI_E_Group[posNA] <- sample(valSameGroup,length(posNA),replace = T)
            PSI_boots[event,,group] <- valPSI_E_Group
          }
        }
      }
    }
...
get_table_Bootstrap <- function(PSI_arrayP,Design,Contrast,nbootstraps,KallistoBootstrap,th){
  ...
  if (KallistoBootstrap == FALSE){ 
      ...
      # save(incr_PSI,file = "incr_PSI.RData")
...
matchRBPs <- function(EventsTable, RBP_Grange){
  positions_star <- c()
  positions_end <- c()
  regions <- c()
  for (event in 1:nrow(EventsTable)) {
    region <- TxtInfo$`Genomic Position`[event]
    region <- strsplit(region,":")[[1]]
    chr <- paste0("chr",region[1])
    regions <- c(regions,chr)
    positions <- as.numeric(strsplit(region[2],"-")[[1]]) + c(-500,500)
    positions_star <- c(positions_star, positions[1])
    positions_end <- c(positions_end, positions[2])
  }
  gr <- GRanges(
    seqnames = regions,
    ranges = IRanges(star=positions_star, end = positions_end, names = EventsTable$EventID))
  
  findRBPs <- findOverlaps(gr,RBP_Grange)
  
  factorRBPsTotal <- factor(RBP_Grange@elementMetadata$RBP,
                            levels = unique(RBP_Grange@elementMetadata$RBP))
  Eventos <- factor(EventsTable$EventID,
                    levels = unique(EventsTable$EventID))
  ExS <- sparseMatrix(i = as.numeric(Eventos[queryHits(findRBPs)]), 
                      j = as.numeric(factorRBPsTotal)[subjectHits(findRBPs)],
                      x = 1,
                      dims = c(length(levels(Eventos)), length(levels(factorRBPsTotal))))
  colnames(ExS) <- levels(factorRBPsTotal)
  rownames(ExS) <- levels(Eventos)
  return(ExS)
}
