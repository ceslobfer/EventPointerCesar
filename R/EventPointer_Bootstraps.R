...
EventPointer_Bootstraps <- function(PSI_boots, Design, Contrast, cores=1,ram=0.1, nBootstraps=10000,
                                    UsePseudoAligBootstrap=TRUE, Threshold = 0){
  
  
  if(is.null(PSI_boots)){
  ...
  if (result == TRUE){
    ...
    table <- mclapplyPSI_Bootstrap(PSI_boots = PSI_boots,
                                   Design = Design,
                                   Contrast = Contrast,
                                   cores = cores,
                                   ram = ram,
...
EventPointerBAM_Bootstraps_BAM<- function(Events, Design, Contrast, cores = 1, ram = 0.1, nBootstraps = 10000,
                                   UsePseudoAligBootstrap = TRUE, Threshold = 0, lambda = NULL, nboot = 20){
  PSI_boots <- getPSI_RNASeq_boot(Events, lambda = lambda, cores, nboot)
  res <- EventPointer_Bootstraps(PSI_boots, Design, Contrast, cores, ram, nBootstraps,
                          UsePseudoAligBootstrap, Threshold)
  return(res)
